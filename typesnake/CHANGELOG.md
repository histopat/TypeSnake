...
- create-vite ile React projesi oluşturun: `npm init --yes vite@latest typesnake -- --template=react-ts  --no-rolldown --no-immediate`..
- `vite.config.ts` dosyası bağlantı noktasıyla güncelleştiriliyor.
- Proje dosyasını (`typesnake.esproj`) oluşturun.
